...
- **`IContainerBuilder` 扩展类命名**：Feature 专属的 Builder 扩展类统一命名为 `{FeatureName}BuilderExtensions`，扩展方法统一命名为 `Register{FeatureName}`；`FeatureName` 采用 PascalCase（多字母缩写混合大小写，如 `Mvvm`、`Fsm`，而非 `MVVM`、`FSM`）
...
- 读 `Documentation/` 下对应模块的设计文档
- 读 `Assets/Ruka/` 下对应模块的代码实现
