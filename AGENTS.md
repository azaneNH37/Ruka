...
- `script-update` — 创建或更新 .cs 文件
- `script-read` — 读取脚本内容
- `code-run` — Roslyn 动态编译执行 C# 代码（主线程操作需用 MainThread.Instance.Run）
- `logs-get` — 读取 Console 日志

### 其余工具暂不可用

因 ReflectorNet schema bug（PR #77 未合并），其他工具调用会报
Invalid schema 错误。遇到错误时，用 code-run 实现等效操作，不要重试。
...
- `.cs` 文件通过 `script-update` 创建；如直接写文件，完成后必须调用 `assets-refresh`
